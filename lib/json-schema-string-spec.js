...
describe("JSON Schema", () => {
  ...
  describe('a schema with `"minLength": 3`', () => {
    const schema = JsonSchema("#", `{ "minLength": 3 }`);
  ...
  describe('a schema with `"pattern": "^[abc]+$"`', () => {
    const schema = JsonSchema("#", `{ "pattern": "^[abc]+$" }`);

    it("should be true for a string that matches the pattern", () => {
      expect(JsonSchema.validate(schema)("aabccc")).to.eql(true);
    });

    it("should not be true if the string does not match the pattern", () => {
