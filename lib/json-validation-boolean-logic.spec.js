...
describe("JSON Schema", () => {
  ...
  describe("not", () => {
    ...
    before(async () => {
      const url = "/not";
      loadSchema(url, { "not": { "type": "number" } });
      const schema = await JsonValidation.get(testDomain + url);
      validate = await JsonValidation.validate(schema);
    });
