...
describe("JSON Validation", () => {
  ...
  describe("patternProperties validates properties matching a regex", () => {
    ...
    before(async () => {
      ...
      const doc = await JVal.get(testDomain + "/patternProperties-validation", JVal.nil);
  ...
  describe("multiple simultaneous patternProperties are validated", () => {
    ...
    before(async () => {
      ...
      const doc = await JVal.get(testDomain + "/multiple-simultaneous-patternProperties", JVal.nil);
      validate = await JVal.validate(doc);
    });
  ...
  describe("regexes are not anchored by default and are case sensitive", () => {
    ...
    before(async () => {
      ...
      const doc = await JVal.get(testDomain + "/regexes-not-anchored-and-not-case-sensitive", JVal.nil);
      validate = await JVal.validate(doc);
    });
