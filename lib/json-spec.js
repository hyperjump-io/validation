...
const append = (schema, keyword) => {
  const ptr = pointer(schema);
  return "#" + encodeURI(JsonPointer.append(ptr, keyword));
};
...
const _validate = async (schema, memo) => {
  if (!memo[schema.url]) {
    ...
    const validatorList = await Promise.all(Object.keys(value(schema))
      .filter((keyword) => validators[keyword])
      .map(async (keyword) => {
        const url = append(schema, keyword);
        return validators[keyword](await get(url, schema), memo);
      }));
...
const validators = {
  ...
  "tupleItems": async (schema, memo) => {
    const ptr = pointer(schema);
    const tupleItems = await Promise.all(value(schema)
      .map(async (_, ndx) => {
        const url = append(schema, ndx);
        const itemSchema = await get(url, schema);
        return _validate(itemSchema, memo);
      }));
  ...
  "properties": async (schema, memo) => {
    ...
    const propertyValidators = new Map(await Promise.all(Object.keys(propertySchemas)
      .map(async (propertyName) => {
        const url = append(schema, propertyName);
        const propertySchema = await get(url, schema);
    ...
    return (value) => [
      ...
      !isObject(value) || Object.keys(value)
        .filter((propertyName) => propertyValidators.get(propertyName))
        .every((propertyName) => {
  ...
  "patternProperties": async (schema, memo) => {
    ...
    const validators = await Promise.all(Object.keys(patternProperties)
      .map(async (propertyPattern) => {
        const url = append(schema, propertyPattern);
        const propertySchema = await get(url, schema);
  ...
  "allOf": async (schema, memo) => {
    const ptr = pointer(schema);
    const allOf = await Promise.all(value(schema)
      .map(async (_, ndx) => {
        const url = append(schema, ndx);
        const subSchema = await get(url, schema);
        return _validate(subSchema, memo);
      }));
  ...
  "anyOf": async (schema, memo) => {
    const ptr = pointer(schema);
    const anyOf = await Promise.all(value(schema)
      .map(async (_, ndx) => {
        const url = append(schema, ndx);
        const subSchema = await get(url, schema);
        return _validate(subSchema, memo);
  ...
  "oneOf": async (schema, memo) => {
    const ptr = pointer(schema);
    const oneOf = await Promise.all(value(schema)
      .map(async (_, ndx) => {
        const url = append(schema, ndx);
        const subSchema = await get(url, schema);
        return _validate(subSchema, memo);
