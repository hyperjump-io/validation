...
describe("JSON Schema", () => {
  ...
  describe("integer type matches integer", () => {
    let validate;

    before(async () => {
      const url = "/integer-type-matches-integer";
      loadSchema(url, { "type": "integer" });
      const schema = await JsonSpec.get(testDomain + url);
      validate = await JsonSpec.validate(schema);
    });

    it("an integer is an integer", () => {
      const result = validate(1);
      expect(ValidationResult.isValid(result)).to.eql(true);
    });

    it("a float is not an integer", () => {
      const result = validate(1.1);
      expect(ValidationResult.isValid(result)).to.eql(false);
    });

    it("a string is not an integer", () => {
      const result = validate("foo");
      expect(ValidationResult.isValid(result)).to.eql(false);
    });

    it("a string is still not an integer, even if it look like one", () => {
      const result = validate("1");
      expect(ValidationResult.isValid(result)).to.eql(false);
    });

    it("an object is not an integer", () => {
      const result = validate({});
      expect(ValidationResult.isValid(result)).to.eql(false);
    });

    it("an array is not an integer", () => {
      const result = validate([]);
      expect(ValidationResult.isValid(result)).to.eql(false);
    });

    it("a boolean is not an integer", () => {
      const result = validate(true);
      expect(ValidationResult.isValid(result)).to.eql(false);
    });

    it("null is not an integer", () => {
      const result = validate(null);
      expect(ValidationResult.isValid(result)).to.eql(false);
    });
  });
  ...
  describe("object type matches objects", () => {
    ...
    it("an integer is not an object", () => {
      const result = validate(1);
      expect(ValidationResult.isValid(result)).to.eql(false);
    });
  ...
  describe("boolean type matches booleans", () => {
    ...
    it("an integer is not a boolean", () => {
      const result = validate(1);
      expect(ValidationResult.isValid(result)).to.eql(false);
    });
